...
#define R(x, n)         ((uint64_t)(x) >> (uint64_t)(n))
...
static void sha512_compress(sha512_ctx *ctx, const unsigned char *buf)
{
	...
	for (i = 0; i < 16; i++) {
		W[i] = _byteswap_uint64(((uint64_t*)buf)[i]);
	}
	// fill W[16..79]
	for (i = 16; i < 80; i++) {
...
void sha512_done(sha512_ctx *ctx, unsigned char *out)
{
	...
	// store length
	((uint64_t*)ctx->buf)[15] = _byteswap_uint64(ctx->length);
	sha512_compress(ctx, ctx->buf);

	// copy output
	for (i = 0; i < 8; i++) {
		((uint64_t*)out)[i] = _byteswap_uint64(ctx->hash[i]);
	}
}
