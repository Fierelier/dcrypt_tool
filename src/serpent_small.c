...
typedef void (*sbox_p)(uint32_t b[4]);

static void sE1(uint32_t b[4])
{
	uint32_t t = b[1];
	b[3] ^= b[0]; 
	b[1] &= b[3];
...
static void __declspec(noinline) KXf(uint32_t *keys, uint32_t round, uint32_t b[4])
{
	b[0] ^= keys[4*round  ];
	b[1] ^= keys[4*round+1];
	b[2] ^= keys[4*round+2];
...
void serpent256_set_key(const unsigned char *key, serpent256_key *skey)
{
	...
	__movsb((unsigned char*)&skey->expkey, (unsigned char*)(W+8), (SERPENT_EXPKEY_WORDS * sizeof(uint32_t)));
}
